...
├── data/
│   ├── generate_data.py          # Generates customer_intents.csv synthetically
│   └── customer_intents.csv      # Synthetic dataset containing text & intent labels
├── models/
│   └── tfidf_intent_pipeline.joblib # Serialized Scikit-learn Pipeline
├── src/
│   ├── __init__.py
│   ├── cleaner.py                # Text normalization and regex cleaner module
│   ├── train.py                  # Model training, evaluation, and serialization script
│   └── app.py                    # FastAPI web server and request validation schemas
├── tests/
│   ├── __init__.py
│   ├── test_cleaner.py           # Unit tests for text normalization and edge cases
│   └── test_api.py               # Integration tests for FastAPI endpoints (/health, /predict)
├── .gitignore                    # standard Python/venv/cache ignore rules
├── requirements.txt              # Exact production & testing dependencies
...
### 5.1. Data Generation (`data/generate_data.py`)
...
### 5.2. Text Cleaning Module (`src/cleaner.py`)
...
### 5.3. Model Training & Serialization (`src/train.py`)
...
### 5.4. Web API Service (`src/app.py`)
...
cd intent-classifier-assessment
...
### Step 3: Train & Serialize ML Model

```bash
python src/train.py
```

### Step 4: Run FastAPI Inference Server
...
## 7. Sample API Usage & Visual Verification
...
curl -X 'POST' \
  ...
  -d '{"text": "My application keeps crashing every time I enter my password."}'
```
...
## 8. Technical Decisions & Trade-off Analysis
...
## 9. Future Improvements (Post-3-Hour Roadmap)
